...
def save_submission(
    ...
    pd.DataFrame(
        ...
        submission_path
        / f"submission-single-modality-baseline-with-{name}-e{epoch}-data.csv",
        index=False,
    )
    ...
    np.save(
        tmp_path
        / f"all-predictions-single-modality-baseline-with-{name}-e{epoch}-data.npy",
        all_predictions_np,
    )

    np.save(
        tmp_path / f"surveys-single-modality-baseline-with-{name}-e{epoch}-data.npy",
        surveys,
    )
...
with open("submission-all-baselines.csv", "w") as f:
